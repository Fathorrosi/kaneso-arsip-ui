...
export class DepartmentFormComponent implements OnInit {
  ...
  onCreate() {
    ...
    this.http
      .post(environment.apiUrl + '/departments', data, { headers })
      .subscribe(
        (response: any) => {
          if (response) {
  ...
  onUpdate() {
    ...
    this.http
      .put(environment.apiUrl + '/departments/' + this.data.id, data, { headers })
      .subscribe(
        (response: any) => {
          if (response) {
            this.handleSuccessResponse();
  ...
  private handleErrorResponse(error) {
    ...
    if (errorObject.message) {
      ...
      if (errorMessage.toLowerCase().includes('duplicate')) {
        errorMessage = 'Nama departemen sudah ada';
      }
    } else {
