...
export class DepartmentComponent implements OnInit {
  ...
  openDialog(params) {
    ...
    this.dialogRef = this.dialog.open(DepartmentFormComponent, config);
    ...
    this.dialogRefConfirm.afterClosed().subscribe((result) => {
      if (result) {
        ...
        const deleteRequests = selectedRows.map((row) => {
          return this.http.delete(environment.apiUrl + '/departments/' + row.id, {
            headers,
          });
        });
