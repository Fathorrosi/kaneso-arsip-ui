...
@Component({
  ...
  styleUrls: ['./major-form.component.scss']
})
export class MajorFormComponent implements OnInit {
  ...
  onCreate() {
    ...
    this.http
      .post(environment.apiUrl + '/majors', data, { headers })
      .subscribe(
        (response: any) => {
          if (response) {
  ...
  onUpdate() {
    ...
    this.http
      .put(environment.apiUrl + '/majors/' + this.data.id, data, { headers })
      .subscribe(
        (response: any) => {
          if (response) {
            this.handleSuccessResponse();
  ...
  private handleErrorResponse(error) {
    ...
    if (errorObject.message) {
      ...
      if (errorMessage.toLowerCase().includes('duplicate')) {
        errorMessage = 'Nama jurusan sudah ada';
      }
    } else {
