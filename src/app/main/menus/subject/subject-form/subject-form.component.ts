...
export class SubjectFormComponent implements OnInit {
  ...
  initializeForUpdate() {
    this.isUpdate = true;
    this.isDetail = false;
    this.title = 'Update Mata Pelajaran';
    this.initializeForm();
  }
  ...
  onCreate() {
    ...
    this.http
      .post(environment.apiUrl + '/subjects', data, { headers })
      .subscribe(
        (response: any) => {
          if (response) {
  ...
  onUpdate() {
    ...
    this.http
      .put(environment.apiUrl + '/subjects/' + this.data.id, data, { headers })
      .subscribe(
        (response: any) => {
          if (response) {
            this.handleSuccessResponse();
  ...
  private handleErrorResponse(error) {
    ...
    if (errorObject.message) {
      ...
      if (errorMessage.toLowerCase().includes('duplicate')) {
        errorMessage = 'Nama mata pelajaran sudah ada';
      }
    } else {
